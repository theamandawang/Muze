...
describe('Auth Session Management', () => {
  it('returns an updated token when an access token is available', async () => {
    ...
    const refreshedToken = await authOptions.callbacks.jwt({
      token: mockToken,
      account: {
          access_token: 'new_token', expires_at: Math.floor(Date.now() / 1000) + 3600,
          providerAccountId: '',
          provider: '',
          type: 'email'
      },
      user: { id: 'user123', name: 'Test User', email: 'test@muze.com', emailVerified: null }
    });
  ...
  it('creates a valid session object with user details', async () => {
    ...
    const session = await authOptions.callbacks.session({ session: mockSession, token: mockToken, user: {
        id: 'user123', name: 'Test User', email: 'test@muze.com',
        emailVerified: null
    }, newSession: true, trigger: 'update' });
