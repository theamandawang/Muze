...
export default function SearchModal({ searchMode, closeModal }: { searchMode: boolean, closeModal: () => void }) {
    ...
    if (!searchMode){
        return (
            ...
            >
                <div className="w-full max-w-2xl sm:w-[80%] p-6 rounded-2xl shadow-lg relative bg-search-purple max-h-[90vh] min-h-[400px] overflow-y-auto"
                ...
                >
                    ...
                    {selectedTrack ? (
                        <AddReviewView track={selectedTrack} onBack={() => setSelectedTrack(null)} />
                    ) : (
                        <div className="flex flex-col items-left p-10">
                            <SearchTracksView onTrackSelect={setSelectedTrack} />
                        </div>
                    )}
                </div>
            </div>
