...
export async function getTopSongs(uid: string) {
    const { data, error } = await SupabaseClient
        .from('top_songs')
        .select('rank, created_at, songs: song_id (spotify_id, album_id, title, img)')
        .eq('user_id', uid)
...
export async function updateTopSong(uid: string, song_id: string, position: number) {
    ...
    if (existingEntry) {
        ...
        const { error: insertError } = await SupabaseClient
            .from('top_songs')
            .insert([{ user_id: uid, song_id, rank: position, created_at: new Date().toISOString() }]);
